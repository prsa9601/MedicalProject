    ...
    public class EditModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostAsync()
        {
            ...
            try
            {
                ...
                if (nationalCardPhoto != null && nationalCardPhoto.Length > 0)
                {
                    nationalCardPhotoPath = await UploadFile(nationalCardPhoto, "national-cards");
                }

                if (birthCertificatePhoto != null && birthCertificatePhoto.Length > 0)
                {
                    birthCertificatePhotoPath = await UploadFile(birthCertificatePhoto, "birth-certificates");
                }

                if (userAccountImage != null && userAccountImage.Length > 0)
                {
                    userAccountImagePath = await UploadFile(userAccountImage, "profile-images");
                }
